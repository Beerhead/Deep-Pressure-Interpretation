...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.btn1 = QPushButton('Интерпретация')
        self.btn2 = QPushButton('Замеры ОТС')
        self.btn3 = QPushButton('Сдвинуть глубину')
        ...
        self.hlGraphLayout.addWidget(self.plotWidget)
    ...
    def connectUI(self):
        self.btn1.clicked.connect(lambda: self.interpretateResearches(method=self.method))
        self.btn2.clicked.connect(lambda: self.measureWidget.show())
        self.btn3.clicked.connect(self.choosePartToOffset)
        ...
        self.plotWidget.btnSpuskPressedSignal.connect(lambda: self.addInfLine(0))
        self.plotWidget.btnPodemPressedSignal.connect(lambda: self.addInfLine(1))
        self.plotWidget.lineSignalToMain.connect(self.infLineMoved)
    ...
    def choosePartToOffset(self):
        if len(self.testList.selectedIndexes()) == 0: return
        index = self.testList.selectedIndexes()[0].row()
        res = self.researchsList[index]
        if not res.interpreted: return
        self.plotWidget.hideInfLines(True)
        self.leftShelfBtn, self.rightShelfBtn = QPushButton("На спуске"), QPushButton("На подъеме")
        self.btnLay = QHBoxLayout()
        self.btnLay.addWidget(self.leftShelfBtn)
        self.btnLay.addWidget(self.rightShelfBtn)
        self.btn3.setDisabled(True)
        self.btn3.setVisible(False)
        self.GL.addLayout(self.btnLay, 2, 1)
        self.leftShelfBtn.clicked.connect(lambda: self.offsetDepthOnPlot(0))
        self.rightShelfBtn.clicked.connect(lambda: self.offsetDepthOnPlot(1))
    ...
    def offsetDepthInResearch(self):
        index = self.testList.selectedIndexes()[0].row()
        res = self.researchsList[index]
        if self.sideToOffset == 0:
            indexes = range(res.centralDots[1])
        else:
            indexes = range(res.centralDots[1], res.finalData.iloc[:, 3].count())
        for i in indexes:
            res.finalData.iloc[i, 3] = res.finalData.iloc[i, 3] + self.timeDelta

        delta = int(self.listModel.item(index, 2).text())
        offsetSplitedData = res.OffsetAndSplitting(res.finalData)
        dataList, res.timesList = res.makeDataForModels(offsetSplitedData, delta)
        res.finalCalc(dataList, res.timesList, offsetSplitedData)

        del (self.sideToOffset, self.timeDelta, self.endPoint, self.initPoint)
        self.plotWidget.hideInfLines(False)
        self.graf()


    def firstClickOnPlot(self, event):
        if event.button() != 1: return
        self.vBox = self.plotWidget.plotItem.getViewBox()
        self.initPoint = pd.Timestamp.fromtimestamp(self.vBox.mapSceneToView(event.pos()).x())
        print('init = ', self.initPoint)
        self.offsetDepthLine.setPos(self.initPoint.timestamp())
        self.plotWidget.addItem(self.offsetDepthLine)
        self.plotWidget.scene().sigMouseClicked.disconnect(self.firstClickOnPlot)
        self.plotWidget.scene().sigMouseMoved.connect(self.onMouseMoveWhileOffsettingDepth)
        self.plotWidget.scene().sigMouseClicked.connect(self.secondClickOnPlot)


    def secondClickOnPlot(self, event):
        self.endPoint = pd.Timestamp.fromtimestamp(self.vBox.mapSceneToView(event.pos()).x())
        print('end  = ', self.endPoint)
        self.timeDelta = self.endPoint - self.initPoint
        print('delta= ', self.timeDelta)
        self.btn3.setDisabled(False)
        self.plotWidget.removeItem(self.offsetDepthLine)
        self.plotWidget.scene().sigMouseMoved.disconnect(self.onMouseMoveWhileOffsettingDepth)
        self.plotWidget.scene().sigMouseClicked.disconnect(self.secondClickOnPlot)
        del(self.offsetDepthLine)
        self.offsetDepthInResearch()


    def onMouseMoveWhileOffsettingDepth(self, event):
        if self.offsetDepthLine is None: return
        curPoint = self.vBox.mapSceneToView(event).x()
        self.offsetDepthLine.setPos(curPoint)
    ...
    def addInfLine(self, polka):
        ...
        line = self.plotWidget.makeInfLine(time, polka)
        self.plotWidget.plotItem.addItem(line)
        line.infLineSignal.connect(self.plotWidget.emittingToMain)
    ...
    def infLineMoved(self, line, start, stop):
        ...
        try:
            ...
            new=True
        checks = [False]
        model = self.researchsList[tempInd].tableModels[line.polka]
        ...
        if new:
            ...
            index = list(depths).index(depth)
            checks.insert(index, True)

        self.researchsList[tempInd].tableModels[line.polka], *_ = self.researchsList[tempInd].makeModel(data, checks=checks)
        self.graf()
    ...
    def keyPressEvent(self, e):
        ...
        #     self.researchsList[0].makeHalfs()
    ...
    def stopGif(self, newResList):
        ...
        for i, res in enumerate(self.researchsList):
            if res.interpreted:
                ...
                self.listModel.item(i, 0).setIcon(self.style().standardIcon(11))
        self.method = not self.method
        self.btn1.setText("Повторная интерпретация")
        self.btn3.setDisabled(False)
        ...
        if self.researchsList[tempInd].interpreted:
            ...
            for num, m in enumerate(self.researchsList[tempInd].tableModels):
                ...
                try:
                    ...
                    ros.append(0)
                ppls.append(round(float(m.item(m.rowCount() - 1, 2).text()) + delta * ros[-1] / 10, 3)) #TODO fix bug here
                checkedSupportTimes.append(halfData)

            if (self.focusWidget() == self.calcTable1 and self.RB1.isChecked()) or \
                (self.focusWidget() == self.plotWidget and self.RB1.isChecked()) or (self.focusWidget() == self.RB1):
            ...
            if self.focusWidget() == self.calcTable2 and self.RB2.isChecked() or\
                (self.focusWidget() == self.plotWidget and self.RB2.isChecked()) or (self.focusWidget() == self.RB2):
            ...
            self.resLabel.setText('Данные по скважине' + '\n' +
                                  ...
                                  'Пласт(ы) - ' + str(layers) + '\n')
            self.plotWidget.plot(self.researchsList[tempInd].data)
            self.calcTable1.setModel(QStandardItemModel())
            self.calcTable2.setModel(QStandardItemModel())
    ...
    def reports(self):
        ...
        for i, res in enumerate(self.researchsList):
            ...
            fig = res.finalFig
            fig.seek(0)
            model = res.tableModels[res.tableInd]
            tableData = []
            checked = []
            ...
            well = res.wellName
            field = res.field
            layer = ''.join(res.layer)
            researchDate = res.researchDate
            vdp = res.vdp
            vdpElongation = res.vdpElong
            ro = res.ro
            ppl = res.ppl
...
class GifDialog(QWidget):
    """Widget, showing gif while interpreting in other thread"""
    def __init__(self):
...
class InterpretationThread(QtCore.QThread):
    """Thread, managing interpreting measurements"""
    finishSignal = pyqtSignal(object)

    def __init__(self, parent=None, pplIntervalDict=None, withML=True, method=0):
    ...
    def run(self):
        ...
        keysList = list(self.pplIntervalDict.keys())
        for res in keysList:
        ...
        self.method = not self.method
        for i, res in enumerate(keysList):
            if res.interpreted and not res.warning: continue
            res.setWarning(False)
            res.interpreted = False
            interval = self.pplIntervalDict[res]
            try:
                newResearchList[i] = res.interpret(interval, self.pModel, self.dModel, self.method)
            except:
                #raise
                newResearchList[i] = res
                print(res.wellName, res.field)
